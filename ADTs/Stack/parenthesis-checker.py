...
def par_checker(symbol_string):
    ...
    closing_braces = [ch for ch in ")}]"]
    # create a mapping for opening and closing braces
    match = {k: v for k, v in zip(opening_braces, closing_braces)}
    s = Stack()
    for symbol in symbol_string:
        if symbol in opening_braces:
            s.push(symbol)
        elif symbol in closing_braces:
